...
@mens_stores.each { |x| puts "#{x.name} the store has a annual revenue of $#{x.annual_revenue}."}

@womens_stores = Store.where(["womens_apparel = ? and annual_revenue > ?", "true", "1000000"])

@womens_stores.each { |x| puts "#{x.name} the store has a annual revenue of $#{x.annual_revenue}." }